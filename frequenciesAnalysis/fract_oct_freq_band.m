...
% This program calculates the fractional, 1/N octave band center frequencies and the lower and upper bandedge limits of every frequency band from min_f to max_f (Hz).
...
% The frequency band nominal values are rounded to three or more significant digits based on the number of bands per octave.  
...
% fract_oct_freq_band produces exact and nominal frequency bands which satisfy ANSI S1.6-R2006 and ANSI S1.11-R2014. 
...
% SI_prefixes = 0;  % This parameter controls the output for the character strings of the nominal frequencies.
...
% Estimate the number of bands being output and return a warning if the array is too large. 

Nbands_total = ceil(N.* (log(max_f) - log(min_f))./ log(G));

if Nbands_total > 10 ^ 6
    
    warning('output array estimated to be larger than 10^6 elements')
...
% Calculate the lower and upper bounds for the lower and upper band edge frequencies
...
% Calculate the number of significant digits for rounding the exact  frequencies into the nominal frequencies.
...
% If the center frequency rounded to 3 significant digits and the last digit rounded to the nearest multiple of 5 is within 1% of the center 
% frequency rounded to 1 significant digit, then round to 1 significant digit. 
...
% Apply the same rounding technique to the lower and upper frequency bandedge limits as was applied to the center frequencies.   
...
% Form the numeric and string arrays for the nominal lower frequency band edge limits.
fl_nom = flu(1 : num_bands);
fl_nom_str = flu_str(1 : num_bands);

% Form the numeric and string arrays for the nominal upper frequency band edge limits.
